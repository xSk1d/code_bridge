...
Inside `CCB-Control`, `Alt-r` rotates pane focus, `Ctrl-]` rotates the active target, and `Enter` sends the current input line directly into that target pane. By default it starts on `gemini`.
