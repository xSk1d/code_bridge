...
In tmux, the default Gemini team layout is: top row `Gemini | Claude | Codex`, with a small `CCB-Control` bar on the bottom.
Inside `CCB-Control`, `Alt-r` rotates pane focus, `Ctrl-]` rotates the active target, `Enter` sends the current input line directly into that target pane, and `Ctrl-o` re-sends role instructions to all worker panes. By default it starts on `gemini`.
